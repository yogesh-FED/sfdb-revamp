...
const SchemeEligibilityPage = (languageData) => {
  ...
  const [no_scheme, set_no_scheme] = useState(false);
     ...
     set_loading(true);
      ...
      if(response.data) {
            if(response.success){
              ...
              set_eligible_schemes(response.data.eligible_schemes.schemes);
              set_scheme_count(response.data.eligible_schemes.scheme_count);
          ...
          languageData.language === "EN" ? ( // Use ? for the true case
              <List dividersIos outlineIos strongIos className='scheme-list'>
                  {eligible_schemes?.map((scheme, index) => (
                      <ListItem  title={`${index + 1}. ${scheme.scheme_name}`} after={scheme_count == 1 ? applyButton(scheme.url) : ""} key={index}>
                          {/* <Icon md="material:done_outline" ios="f7:checkmark_alt" slot="media" />  */}
                      </ListItem>
                  ))}
              </List>
              ...
              <List dividersIos outlineIos strongIos className='scheme-list'>
                  {eligible_schemes?.map((scheme, index) => (
                      <ListItem  title={`${index + 1}. ${scheme.scheme_name_tamil}`} after={scheme_count == 1 ? applyButton(scheme.url) : ""} key={index}>
                          {/* <Icon md="material:done_outline" ios="f7:checkmark_alt" slot="media" />  */}
                      </ListItem>
                  ))}
              </List>
